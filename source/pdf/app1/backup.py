...
def chat_question(elements, fn, ln):
    user = User.objects.get(first_name=fn, last_name=ln)
...
def func(request):
    elements = []
    user = User.objects.get(first_name="Jhon", last_name="Taylor")
    filename = str(str(user.id)+".pdf")
    doc = SimpleDocTemplate(filename, pagesize=landscape(A4))
    elements = basic_info("Jhon", "Taylor", elements)  # ,pagesize)
    elements = other_info(elements, "Jhon", "Taylor")
    elements = chat_question(elements, "Jhon", "Taylor")
