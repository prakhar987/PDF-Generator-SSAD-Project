...
def generate_data_for_table(array, elements, i, l, sec, tes):
    while l < len(tes):
        ...
        i1 = 0
        count = 0
        count = fetch_data_belonging_to_a_test(arr, array, count, elements, i, i1, l, tes)
        i1 = 0
        ...
        while i1 < len(sec):
            ...
            while l2 < count:
                count1 = arrange_data_according_to_sub_sections(ar, arr, count1, elements, i1, l2, sec)
                l2 += 1
            styles = ParagraphStyle(
                name='Normal',
            ...
            generate_table_with_fetched_data(ar, count1, elements, i1, sec, styles)
            i1 += 1
        l += 1


def generate_table_with_fetched_data(ar, count1, elements, i1, sec, styles):
    if count1 > 1:
...
def arrange_data_according_to_sub_sections(ar, arr, count1, elements, i1, l2, sec):
    if arr[l2][1] == sec[i1]:
